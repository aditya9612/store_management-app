...
def create_invoice(order):
    ...
    if order.store.address:
        c.drawString(400, 710, f"Address: {order.store.address}")
    ...
    for item in order.items:
        ...
        # Use item.name for both manual and catalog products
        # For catalog products, item.name will be set to product.name in the CRUD
        # For manual products, item.name will be set directly
        product_name = item.name or "Unknown Product"

        # Add description if available
        if item.description:
            product_name += f" ({item.description})"
    ...
    c.drawString(100, y, "Tax Breakdown")
    y -= 20
    c.line(100, y, 500, y)
    y -= 20
    ...
    taxable_after_discount = taxable_amount - discount_amount

    # GST calculations (18% total - 9% CGST + 9% SGST)
    cgst_rate = 0.09  # 9% CGST
    gst_rate = 0.09   # 9% SGST (labeled as GST)

    cgst_amount = taxable_after_discount * cgst_rate
    gst_amount = taxable_after_discount * gst_rate

    # CGST row
    c.setFont("Helvetica", 10)
    c.drawString(100, y, "CGST")
    c.drawString(350, y, "9.0%")
    c.drawString(450, y, f"${cgst_amount:.2f}")
    y -= 20

    # GST row
    c.drawString(100, y, "GST")
    c.drawString(350, y, "9.0%")
    ...
    c.drawString(350, y, f"Taxable Amount: ${taxable_after_discount:.2f}")
    y -= 20

    c.drawString(350, y, f"CGST (9.0%): ${cgst_amount:.2f}")
    y -= 20

    c.drawString(350, y, f"GST (9.0%): ${gst_amount:.2f}")
    y -= 20

    # Total tax
    total_tax = cgst_amount + gst_amount
    c.drawString(350, y, f"Total Tax: ${total_tax:.2f}")
    y -= 20

    # Final total
    final_total = taxable_after_discount + total_tax
