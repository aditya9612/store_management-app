...
def create_store(db: Session, store: schemas.StoreCreate):
    ...
    # Create new store with all address fields
...
def update_store(db: Session, store_id: int, store: schemas.StoreCreate):
    ...
    if db_store:
        ...
        db_store.pincode = store.pincode
        db_store.gstin = store.gstin
...
def create_order(db: Session, order_data: schemas.OrderCreate):
    ...
    if not store:
        raise ValueError(f"Store with id {order_data.store_id} not found")

    # Check if customer exists in this store
    customer = db.query(models.Customer).filter(
        models.Customer.id == order_data.customer_id,
        models.Customer.store_id == order_data.store_id
    ).first()
    if not customer:
    ...
    for item in order_data.items:
        # For catalog items, verify the product exists and get details
        if item.product_id:
            product = db.query(models.Product).filter(
                models.Product.id == item.product_id,
                models.Product.store_id == order_data.store_id
            ).first()
            if not product:
                raise ValueError(f"Product with id {item.product_id} not found in this store")
            ...
            # For manual items, ensure required fields are present
            if not item.name:
                raise ValueError("Name is required for manual items")
            if item.price is None or item.price <= 0:
                raise ValueError("Valid price is required for manual items")
            name = item.name
            price = item.price
    ...
    if order_data.discount and order_data.discount > 0:
        discount_amount = (total * order_data.discount) / 100
