...
export default function InvoicesSection({ invoices, setInvoices, loggedInOwner }) {
  ...
  // ✅ Load invoices and shop details on mount
  useEffect(() => {
    if (ownerId) {
      const storedInvoices = JSON.parse(localStorage.getItem(`invoices_${ownerId}`)) || [];
      setInvoices(storedInvoices);

      const storedShop = JSON.parse(localStorage.getItem(`shop_${ownerId}`));
  ...
  const handleGenerateInvoice = (e) => {
    ...
    const newInvoice = {
      ...
      items,
      total: subtotal,
      shop: shopDetails || {}, // ✅ attach shop info
    };
  ...
  // ✅ Print invoice with shop details
  const printInvoice = (invoice) => {
    ...
    // ✅ Fallback: if invoice.shop is missing, use current shopDetails
    ...
    const html = `
      ...
      <body>
        <div class="header">
          ...
          <p class="shop">
            ${shop.address || ""}<br/>
            Phone: ${shop.phone || ""} | Email: ${shop.email || ""}<br/>
            GST: ${shop.gst || ""}
          </p>
        </div>
  ...
  // ✅ Send SMS (requires backend API at localhost:5000)
